...
class TestHeap(unittest.TestCase):
    ...
    def test_16_is_full_02(self) -> None:
        test_heap = MinHeap(5)
        test_heap.build_heap([1, 2, 3])
        print(test_heap.heap)
        self.assertEqual(test_heap.size(), 3)

    def test_17_get_heap_cap_02(self) -> None:
        test_heap = MinHeap(7)
        self.assertEqual(test_heap.capacity(), 7)
        test_heap.build_heap([2, 9, 7, 6, 5, 8])
        print(test_heap.heap)
        print(f"capacity: {len(test_heap.heap) - 1}")
        self.assertEqual(test_heap.capacity(), 7)

    def test_build_heap_01(self) -> None:
        test_heap = MinHeap(2)
        test_heap.build_heap([2, 9, 7, 6, 5, 8])
        self.assertEqual(test_heap.size(), 6)

    def test_contents_01(self) -> None:
        test_heap = MinHeap(2)
