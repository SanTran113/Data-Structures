...
class TestList(unittest.TestCase):
    ...
    def test_03(self) -> None:
        ...
        # print(conc.concordance_table.hash_table)
    ...
    def test_04(self) -> None:
        conc = Concordance()
        conc.load_stop_table("stop_words.txt")
        conc.load_concordance_table("bear.txt")
        conc.write_concordance("bear_con.txt")
        self.assertTrue(compare_files("bear_con.txt", "bear_sol.txt"))

    def test_05(self) -> None:
        conc = Concordance()
        conc.load_stop_table("stop_words.txt")
        with self.assertRaises(FileNotFoundError):
            conc.load_concordance_table("cat.txt")
            # self.assertTrue(compare_files("cat_con.txt", "cat_sol.txt"))

    def test_06(self) -> None:
        conc = Concordance()
        with self.assertRaises(FileNotFoundError):
            conc.load_stop_table("stopwords.txt")
